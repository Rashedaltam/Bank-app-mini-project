...
const WithdrawFromoAccount = async () => {
  const responce = await instance.put("/mini-project/api/transactions/withdraw");
  return responce.data;
};
...
const TransferfromMYAcc = async () => {
  const responce = await instance.put("/mini-project/api/transactions/transfer/<username>");
  return responce.data;
};
...
export {GetProfile, GetTransactions, GetAllUsers, UpdateProfile,DepositToAccount, WithdrawFromoAccount,TransferfromMYAcc, GetUserInfo};
